...
针对某省八区域电网的多源计量口径差异、跨区碳责任划分、五年项目约束耦合及省外送端碳因子波动，本文构建由数据调和、碳流追踪、项目组合和不确定性分析组成的模型链，全部计算均可由程序复现。问题一在守恒约束下进行标准化 Huber 数据调和，并将年度工程线损作为软先验。23 项异常仅作诊断标记，稳健目标为 291.0288，最大平衡残差为 $4.09\times10^{-13}$\,GWh，E05 线损估计为 0.01675。问题二逐月求解比例共享碳势，并通过 500 次条件参数自举传播量测误差。每次自举均先生成伪观测，再完整执行调和与碳流计算。A4、A5 年均碳强度分别为 0.62399、0.15217\,kgCO$_2$/kWh，A4$\to$A3 碳流点估计为 4366.34\,kt，95\% 条件区间为 $[4356.76,4419.04]$\,kt。$\lambda=0.5$ 与所构造的 Shapley 分配一致，但这来自特征函数的结构恒等式，不能单独证明该权重在政策上最公平。问题三按缺供、投资和最大相对区域超额建立词典序 $\varepsilon$-约束。最低投资 $C^*=964.970$ 百万元；推荐 $\varepsilon=1\%$，对应投资 974.603 百万元，加权基线的最大相对超额由 8.281\% 降至 7.611\%，全部硬约束均通过独立回代。问题四分别检验设计集与统一 $\Gamma$ 集，并区分软违约和硬可行。完整 $\Gamma=1$ 下的最小不可避免缺口为 14.8488\,kt，最大硬可行半径 $\tau^*=0.953003$；四条典型路径均未给定概率，非预见最小最大相对遗憾为 141.595\%，且均满足硬约束。滚动比较采用同一状态下的剩余期目标，2028、2029 年分别改善 307.566、434.874 百万元。该改善包含避免的违约罚，不能等同于投资节省。
...
某省一次能源不足，电力消费对省外输入存在刚性依赖。省内八个区域（记为 $A1,\ldots,A8$）产业结构、电源结构与发展阶段分化明显，区域间由 11 条通道互联。电力来源的碳排放强度差异大，区域用电的碳责任不能只按本地发电核算。2026至2030 年将滚动实施工业节能、建筑改造、交通电气化、分布式光伏及通道升级等 35 个候选项目，项目之间存在资源竞争、互斥与协同。与此同时，省外送端电源结构随季节与供需变化，输入电的碳因子并非常数。
...
\item 识别并校正多源电量，估计通道损耗，评价观测影响度；
\item 追踪碳流，给出逐月终端碳强度，并按发电地、用电地及兼顾二者的口径分担责任；
\item 在供能与全省碳上限下安排五年项目，兼顾投资成本与区域负担，并检验方案稳定性；
\item 评估送端碳因子不确定性对问题三方案的冲击，给出满足约束的调整机制并量化抗风险代价；
\item 面向选定决策主体撰写约 1000 字咨询报告。
\end{enumerate}
附件 6 的题面文字写“突发事件”，Excel 给出的是碳因子上下界与情景 S0至S3，本文以 Excel 为准。
...
技术路线如图\ref{fig:roadmap}所示：附件 1/2 $\to$ 问题一调和 $\to$ 附件 3 与碳势方程 $\to$ 问题二责任与份额矩阵 $S_{ij}$ $\to$ 附件 4/5 的五年 MILP $\to$ 附件 6 的情景与 $\Gamma_t$ $\to$ 咨询报告。计算环境为 Python 3，二次规划使用 cvxpy/Clarabel，MILP 使用 Pyomo/HiGHS。正文中的关键数值均由程序生成并经交叉核验，文字取舍不改变计算口径和数值结果。
...
\begin{tikzpicture}[node distance=0.65cm and 0.78cm, >=Latex,
  ...
  \draw[arr] (q4) -- (q5);
\end{tikzpicture}
\caption{五个问题的计算顺序。调和后的电量依次用于碳流核算、五年项目优化和鲁棒滚动分析。}
\label{fig:roadmap}
\end{figure}
...
上述 23 项异常或口径不一致记录仅作为诊断标记，不能直接认定为粗差。影响度计算对候选观测施加对称 $\pm5\%$ 有限差分，基准解与扰动解均采用同一 Huber 模型。区域年度用电变化和线损率变化先分别标准化，再合成为无量纲分数，因而不会直接混合 GWh 与线损率，结果也不受人为迭代轮数影响。

\subsection{校正结果}

\begin{sloppypar}
Huber 模型达到最优，稳健目标为 291.028826。标准化观测残差平方和为 102.071576，除以 1274 个观测项得到 0.08011898。该值只是描述性 $\mathrm{RSS}/n$：模型另含 106 个先验残差、硬约束、相关观测与估计状态，不宜当作约化卡方，也不宜据此做拟合优度检验。平衡残差最大 $4.09\times10^{-13}$\,GWh，容量、非负性、$r\le s$ 与结构性零流量均满足。四处缺失插补为 48.15、68.73、493.62、881.32\,GWh（依次为 A7 风电、A4 水电、A2 工业、A2 终端用电合计）；全省区内损耗率为 1.742\%。E05 年度线损为 0.01675（工程值 0.02000），E10 为 0.02777。影响度前三依次是 E10 年度送端汇总、E10 年度受端汇总和 2025-08 A4 省外输入，建议优先复核年度与月度关口的统计口径。
\end{sloppypar}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{../src/outputs/q1/fig_q1_raw_residual_heatmap.pdf}
\caption{校正前能量平衡标准化残差。色标截断于 $\pm6$，边际条为各月、各区的 $\max|z|$；A2 九月极端值 $z=32.5$ 另行标明。}
\label{fig:q1res}
\end{figure}
...
\caption{通道线损率对比。空心点、实心点和竖线分别表示工程值、调和估计与原始隐含值；E07 因无有效流量而保留工程值。}
\label{fig:eta}
\end{figure}
...
\label{eq:rho}
\end{equation}
其中 $C_{ij}$ 为 $j\to i$ 到达电量，$b_i$ 为节点 $i$ 本地发电与省外输入注入的碳量。\eqref{eq:rho} 逐月构成 $8\times 8$ 线性方程组。在同一比例共享结构下，以各类注入能量为右端项，一次求得来源份额；所得份额不再额外除以 $Q_i$。省外输入按接入区分列的份额矩阵 $S_{ij}$ 供问题四使用。

\subsection{三种责任口径与 Shapley}

扩展生产责任 $P^+_i$ 等于本地发电碳排放与该区接入的省外输入碳排放之和\cite{Peters2008production}。全口径消费责任 $C^+_i$ 包括终端用电碳排放、区内损耗碳排放和送端承担的通道损耗碳排放\cite{Davis2010consumption}。线性共担\cite{Gallego2005consistent,Lenzen2007shared}
\begin{equation}
...
图\ref{fig:q2season}将同一分解细化到月份：A5/A7 的水电份额在 4至8 月上升，火电与省外输入占比相应下降，与碳势的夏季低谷一致；A2/A4 各月均以火电和省外输入为主，碳势因而全年处于高位。图\ref{fig:q2sankey}以冲积图汇总各类电源到消费区的能量去向：省外输入主要进入 A1/A4/A6，水电集中供给 A5/A7。
...
图\ref{fig:cfnet}进一步给出通道碳流与区域净责任差额，图\ref{fig:resp}则比较四种责任口径。两图共同表明，A4 是主要净转出区域，A3 是主要净转入区域。
...
\caption{八区域年均终端碳强度点估计及 95\% 条件区间。右图采用相对偏差轴，圆点为 P50。}
...
\caption{前十条通道的年度到达碳流点估计及 95\% 条件区间。右图采用相对偏差轴以显示不同量级的区间。}
...
决策变量为项目 $p$ 在年份 $t$ 的开工规模 $x_{p,t}$，其中通道升级项目采用 0-1 变量。运行层包含本地电源上限、省外输入年容量、通道容量与问题一校正后的 $\eta_e$，以及最低供能率。碳核算按附件 4B 的官方简化式
\begin{equation}
...
\label{eq:obj}
\end{equation}
其中 $b_{i,y}=\max(0,E_{i,y}-B_{i,y})$，主口径 $r=0$。式\eqref{eq:obj}中的系数 0.2 人为规定了公平与成本之间的交换率，因此该目标仅作为比较基线。正式方案采用严格词典序 $\varepsilon$-约束：先最小化总缺供 $S$，再在 $S=S^*$ 下最小化投资 $C$，最后在 $C\le(1+\varepsilon)C^*$ 内最小化最大相对区域超额 $R$，并在并列最优解中选择投资较小者：
\begin{equation}
\begin{aligned}
S^*&=\min S(x),\\
C^*&=\min\{C(x):S(x)\le S^*\},\\
...
\label{eq:epsilon}
\end{equation}
该结构仍是多年期投资与运行耦合 MILP\cite{Koltsaklis2018generation}。各阶段均由 HiGHS 求解至最优并独立回代，避免直接加总不同量纲的目标。
...
\label{eq:dE}
\end{equation}
将 $S_{ij}$ 固定在 2025 年水平是一项较强假设。若只计接入区进口电量而忽略转供，S3 情景下 2030 年的增量由 454.5 降至 437.8\,kt（$-3.7\%$）；若再将 $S$ 向 A4 集中 20\%，增量则升至 495.7\,kt。当前模型只描述给定来源份额下的因子冲击，未覆盖 $S$ 的长期漂移。
...
$\tau>1$ 时设计集扩大，而项目与资源上限不变，因而软违约上升；统一 $\Gamma$ 回测下的违约仅由 14.85\,kt 变为 14.89\,kt，基本稳定。$\tau=1$ 的软方案投资 4834.5 百万元，较确定性基线增加 3860.6 百万元，但仍存在 14.85\,kt 缺口。

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{../src/outputs/q4/fig_q4_stress.pdf}
\caption{问题三加权罚基线的越限热力图（情景$\times$年份）。S0 均为零，S3 情景下 2030 年为 454.5\,kt。}
\label{fig:stress}
\end{figure}
...
题面未给 S0至S3 赋予概率，因此不能按等概率计算期望。令 $C_s^*$ 为路径 $s$ 的完美信息（PI）硬碳上限最优目标；令 2026至2027 开工 $x^0$ 对四路径共享，2028至2030 在年初识别路径后用 $x^s$ 追索。采用混合整数追索的最小最大相对遗憾思想\cite{BenTal2004adjustable,Barkom2025regret}：
\begin{equation}
\min R,\qquad
...
\vspace{0.3em}
对 2025 年多源监测数据的守恒校验得到 23 项诊断标记，包括月度与年度系统最大 8.7\% 的口径差、E10 年度“受端大于送端”、E03 单月隐含线损 17.5\% 以及 4 处缺失。采用单次凸 Huber 调和后，最大能量平衡残差为 $4.09\times10^{-13}$\,GWh；E05 年度线损为 0.01675（工程值 0.020），E07 因全年无有效流量只能保留工程值 0.027。影响度排序前三的是 E10 年度送端汇总、E10 年度受端汇总和 A4 八月省外输入。建议统一关口月表与年度汇总的编码，并对 E10 设置自动对账告警。
...
本文对各环节分别作独立回代与对照检验。问题一比较 Huber 与 WLS 在污染数据下的表现；问题二的 500 个自举样本均重新执行上游调和；问题三的五个 $\varepsilon$ 点均单独校验约束。问题四分别报告设计集与统一集、软约束与硬约束、完美信息目标与非预见目标，避免混用评价口径。
